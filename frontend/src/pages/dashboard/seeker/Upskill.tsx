...
export const Upskill = () => {
  ...
  useEffect(() => {
    const loadQuestions = async () => {
        try {
            const { DUMMY_PYQS } = await import('@/data/dummyData');
            console.log("✅ Questions Loaded (Dummy):", DUMMY_PYQS.length);
            // Cast to any since the types in dummyData might differ slightly from CompanyData locally defined
            setAvailableData(DUMMY_PYQS as unknown as CompanyData[]);
        } catch (err) {
